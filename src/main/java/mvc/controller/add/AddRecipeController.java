...
public class AddRecipeController extends AbstractAddController implements InyectableController {
    ...
    public void create(EntryDTO dto) {
        ...
        Logger.getInstance().log(LogLevel.INFO, this.getClass().toString(), "El usuario " + this.context.getAccount().getUsername() + " ha creado una receta llamada " + dto.name + " en la coleccion " + this.context.getCurrentCollection().getName());
    ...
    private void addPopUp(Button button, AddRecipeView view, VBox targetList){
        ...
        List<ItemDTO> listItems = itemDAO.readAll(collectionId.value());
