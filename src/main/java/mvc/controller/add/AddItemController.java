...
public class AddItemController extends AbstractAddController implements InyectableController{
    ...
    public void create(EntryDTO dto) {
        ...
        Logger.getInstance().log(LogLevel.INFO, this.getClass().toString(), "El usuario " + this.context.getAccount().getUsername() + " ha creado un item llamado " + dto.name + " en la coleccion " + this.context.getCurrentCollection().getName());
        goBack();
    }
}